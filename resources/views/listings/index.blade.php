            ...
            <div class="flex-justify-center">
                @foreach($tags as $tag)
                    <a href="{{ route('listings.index', ['tag' => $tag->slug]) }}"
                       class="inline-block ml-2 tracking-wide text-xs font-medium title-font py-0.5 px-1.5 border border-red-500 uppercase {{ $tag->slug === request()->get('tag') ? 'bg-red-700 text-white' : 'bg-white text-white-500' }}"
                    >{{ $tag->name }}</a>
                @endforeach
            </div>
        </div>

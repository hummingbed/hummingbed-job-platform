    ...
    <section class="text-gray-600 body-font overflow-hidden">
        <div class="container px-5 py-12 mx-auto">
            <div class="mb-12 flex items-center">
                <h2 class="text-2xl font-medium text-gray-900 title-font px-10">

                     {{ empty($listings) ? 'You Have No Listing' : 'Your listings' . $listings->count() }}
                </h2>
                <form action="{{ route('logout') }}" method="post">
                    @csrf
                    <button type="submit" class="ml-40 text-indigo-500">Sign Out</button>
                </form>
            </div>
            <div class="-my-6">
                @if(!empty($listings))
